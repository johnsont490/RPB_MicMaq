...
class System: #need to set dict objects to exclude timeseries for metrics
    ...
    def operation(self, operation='normal', critical_load_MW = None, df_res = None, res_initial = None):
        ...
        # fuel properties
        fuels = {
            'LPG': {'heating_value': 46.1, 'efficiency': 0.75},  # fix this later - do some research on this 
        }
...
(NPV_renewables_N / NPV_renewables_L) * (NPV_OM_L + NPV_htr_ARMO_L + NPV_batt_ARMO_L)))) / ((1 + metrics['WACC_r']) ** metrics['analysis_period'])
        ...
        # Cash flow schedule in Analysis Period
    ...
    def system_metrics(self):
        ...
        grid_to_load_Mar1_to_Jun1 = df.grid_to_load_MWh_e[start_index:end_index].sum()
        pct_sys_window = 100*(1-(grid_to_load_Mar1_to_Jun1/load_MWh_Mar1_to_Jun1))
        ...
        metrics['percent_load_by_system_net'] = 100*(metrics['load_MWh']-metrics['grid_to_load_MWh_e']+metrics['PV_System_to_grid_MWh_e'])/metrics['load_MWh']
        metrics['percent_time_50pct_load'] = 100*((df.grid_to_load_MWh_e/df.load_MW)<.5).sum()/len(df)
    ...
    def timeseries_plot_source(self, start_date='random', days=365*2, type='area', color_seq=None, system_seq=None):
        ...
        # Ensure all values in 'grid_to_load_MWh_e' are positive
        df2['grid_to_load_MWh_e'] = df2['grid_to_load_MWh_e'].abs()
        sum_grid = np.sum(df2['grid_to_load_MWh_e'])
        sum_pv = np.sum(df2['ASGARD_PV_to_load_MWh_e'])
        sum_TES = np.sum(df2['ASGARD_TES_to_load_MWh_e'])
        sum_total = sum_grid + sum_pv + sum_TES
        print((sum_pv+sum_TES)/sum_total)
